...
import requests
import base64
from subprocess import call,run,check_output,Popen,PIPE,STDOUT,CalledProcessError
...
def prepare_and_attach_invoice(doc, replace=False):
    ...
    invoice_xml = frappe.render_template(
        "saudi_einvoice/saudi_einvoice/e_invoice.xml",
...
def generate_invoicehash(doc,method):
    ...
    cwd = os.getcwd() 
    file_name = cwd+'/mum128.erpgulf.com'+"/public/files/IT24556_00075.xml"
    with open(file_name,"rb") as f:
        data = f.read()
        sha256hash = hashlib.sha256(data).hexdigest()
    settings = frappe.get_doc('Hash')
    settings.hash = sha256hash
    settings.save()
